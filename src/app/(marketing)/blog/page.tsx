...
export default function BlogPage() {
  ...
  useEffect(() => {
    const fetchBlogPosts = async () => {
      try {
        const response = await get<{ success: boolean; data: BackendBlogPost[] }>('/blog');
        if (response.success && response.data) {
          // Transform backend data to BlogPost format
          const transformedPosts: BlogPost[] = response.data.map(post => ({
            ...
            author: post.author,
            date: new Date(post.publishedAt).toLocaleDateString('ar-EG', {
              year: 'numeric',
              month: 'long',
              day: 'numeric'
            }),
          ...
          // Use first 6 posts as featured
          setFeaturedPosts(transformedPosts.slice(0, 6));
          // Use all posts for latest section
